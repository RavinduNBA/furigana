...
mkdir -p "$ARTIFACTS/run-a" "$ARTIFACTS/run-b" "$ARTIFACTS/disabled" "$ARTIFACTS/invalid" "$ARTIFACTS/failure" "$ARTIFACTS/compatibility"
...
"$PYTHON" -m pytest -q "$ROOT/tests/test_grammar_analysis.py"
