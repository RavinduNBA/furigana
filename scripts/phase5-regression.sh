...
rm -rf "$OUT/run-a" "$OUT/run-b" "$OUT/cache-a" "$OUT/cache-b" "$OUT/cache-failure"
...
.venv/bin/python -m pytest -q tests/test_enrichment.py
echo "Phase 5 request, scripted-cache, and fallback regression passed. Artifacts: $OUT"
